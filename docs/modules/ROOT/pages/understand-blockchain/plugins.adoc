...
// URLs
:url_github_hello_plugin: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/tutorials/hello-world/blockchain_app/hello_plugin/index.js
...
If the application was xref:{url_guides_setup}[bootstrapped] with Klayr Commander, they are registered in the file `src/app/plugins.ts`.
...
The interface for the `BasePlugin` is described in the xref:{url_references_framework_baseplugin}[Klayr Framework reference].
