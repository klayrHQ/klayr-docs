= Network
:description: This section describes the Klayr P2P network used to exchange information such as blocks or transactions between nodes.
...
Klayr nodes connect to each other in a random manner, thereby forming an unstructured P2P network where information is exchanged using a gossip-based protocol.
...
== Klayr P2P architecture

image::protocol/p2p.svg[KlayrP2P2,262,320 ,align="center"]
...
The _Klayr RPC and Events_ layer lies on top of SocketCluster.
This layer manages interactions between nodes using Remote Procedural Calls (see {url_wikipedia_rpc}[Wikipedia page]).

Finally, the _Klayr application_ layer provides a high level interface to interact with the blockchain.
