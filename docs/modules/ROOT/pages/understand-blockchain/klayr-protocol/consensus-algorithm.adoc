...
:url_github_lip_22: https://github.com/KlayrHQ/lips/blob/master/proposals/lip-0022.md
...
This section describes two important areas of the Klayr protocol which together are referred to as consensus algorithm.

The first part describes the protocol for deciding who is allowed to add a new block to the blockchain.
Klayr uses *Delegated Proof-of-Stake (DPoS)*, which means that every Klayr holder can vote for delegates, and depending on these votes certain delegates are allowed to add blocks to the blockchain in a certain order.
Delegates are simply normal Klayr accounts, which performed a xref:{url_transactions_delegate}[delegate registration transaction].

The second part of this section explains the *Klayr-BFT* consensus protocol.
...
Blocks in the Klayr protocol are grouped together into batches of 103 consecutive blocks, which are referred to as a [#index-round-1]#*round*#.
...
In Klayr DPoS, the forger list is computed taking into account the delegate weights and some random values submitted by delegates as part of their block.
The top 101 delegates by delegate weight are called [#index-active_delegate-1]#*active delegates*#, whereas all other delegates with a delegate weight of at least 10^11^ (1000 KLY in Klayr Mainnet) are referred to as [#index-standby_delegate-1]#*standby delegates*#.
...
During the bootstrap period defined in the xref:{url_blocks_genesis}[genesis block] the number of blocks per round and length of the forger list is still 103. However, the Klayr DPoS delegate selection is not used. Instead, the forger list is directly computed from the initial delegates specified in the genesis block, repeating delegates if needed to obtain a list of length 103.
...
The Klayr-BFT protocol is formally defined in the related research paper "A lightweight BFT consensus protocol for blockchains" ({url_arxiv}[arXiv]).
This section provides a high-level overview of the Klayr-BFT protocol and its security guarantees.

[[fork_choice_rules]]
=== Consensus votes on blocks and fork choice rule

The Klayr-BFT protocol uses special [#index-consensus_votes]#*consensus votes*# that the forging delegates cast for blocks.
...
In the Klayr-BFT protocol delegates choose the chain where the block at the tip of the chain has the largest `maxHeightPrevoted` value (and largest `height` value in case of a tie).
...
For the case of a static set of 101 active delegates (i.e., the active delegates are not changing from round to round), the Klayr-BFT protocol provides the following security guarantees:

* If at most 33 active delegates are [#index-Byzantine-1]#*Byzantine*# (malicious), and all other active delegates obey the rules of the Klayr-BFT protocol, two conflicting blocks are never finalized.
* If the active delegates obey the rules of the Klayr-BFT protocol and at most 33 active delegates stop forging or casting consensus votes, new blocks can always be finalized.
* A violation of the Klayr-BFT protocol by a delegate can be detected and the respective delegate can be punished, as outlined in the next section.

In case the set of active delegates changes over time, the security guarantees are only slightly weaker.
For details, refer to the Klayr-BFT research paper ({url_arxiv}[arXiv]).

[[punishment]]
=== Punishment of Klayr-BFT protocol violations
...
Once this transaction is included in the blockchain, the delegate that violated the Klayr-BFT consensus protocol and forged the two blocks is punished by setting the delegate weight to 0 for the next 780,000 blocks (approximately 3 month).
