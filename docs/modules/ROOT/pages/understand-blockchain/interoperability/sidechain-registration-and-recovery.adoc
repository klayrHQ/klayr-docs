...
//URLs
:url_lip_31_rmt: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0031.md#regular-merkle-trees
:url_lip_45: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0045.md
:url_lip_51_recoverevent: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0051.md#recover-1
:url_lip_45_terminatechain: {url_lip_45}#terminatechain
:url_lip_53: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0053.md#verification
:url_typedoc_interop_module: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.SidechainInteroperabilityModule.html
:url_typedoc_interop_init_recovery: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.InitializeStateRecoveryCommand.html
:url_typedoc_interop_register_sidechain: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.RegisterSidechainCommand.html
:url_typedoc_interop_register_mainchain: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.RegisterMainchainCommand.html
:url_typedoc_interop_message_recovery: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.RecoverMessageCommand.html
:url_typedoc_interop_state_recovery: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.RecoverStateCommand.html
:url_typedoc_interop_terminate4liveness: {site-url}/klayr-sdk/v6/references/typedoc/classes/klayr_framework.TerminateSidechainForLivenessCommand.html
//Project URLs
...
====
On this page, you learn about the sidechain registration and recovery process in the Klayr ecosystem.
...
//TODO: Add link to Klayr Mainchain page once it is created
The first step for a Klayr blockchain to become interoperable with other sidechains and the Klayr Mainchain is to register itself to the mainchain.

A sidechain is registered by performing the following steps:

. <<sidechain-registration-on-the-klayr-mainchain>>
...
The {url_typedoc_interop_register_sidechain}[Register Sidechain^] command is used to register a sidechain on the Klayr mainchain.
...
The {url_typedoc_interop_register_mainchain}[Register Mainchain^] command is used to register the Klayr mainchain on a sidechain.
...
. The sidechain is <<violating-the-klayr-interoperability-protocol>>.
...
For example, the Token module on the mainchain can terminate a sidechain if it violates escrow, i.e. sends more KLY tokens to the mainchain that were sent to this sidechain.
...
==== Violating the Klayr Interoperability protocol

It is also possible for a sidechain to be terminated, if it violates the Klayr Interoperability protocol.
Note, that if a sidechain uses the Klayr Interoperability module, it never violates the protocol.
...
For example, in the case whereby escrowed KLY tokens exist in the sidechain account on the mainchain, which could have been subtracted by prior malicious behavior in the terminated sidechain.
+
...
On the mainchain in the Klayr *Mainnet*, the own chain account is present by default and set to an object with the following properties:
...
For the mainchain account on a sidechain, this property is initialized to the string `"klayr_mainchain"`.
...
An entry for the Klayr Mainchain is present by default.
Any new entry for a sidechain is created as part of the <<sidechain-registration-on-the-klayr-mainchain>> in the "registered names" substore.
