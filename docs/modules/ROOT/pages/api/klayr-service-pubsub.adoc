...
The blockchain update API can be accessed by the following path `wss://service.klayr.com/blockchain`.

If you are interested in accessing the testnet network, this can be accomplished by using the `wss://testnet-service.klayr.com/blockchain` endpoint.
...
const connection = io.connect('wss://service.klayr.com/blockchain', { transports: ['websocket'] });
