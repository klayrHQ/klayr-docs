...
// Project URLs
:url_general_endpoints: understand-blockchain/endpoints.adoc
:url_modules: {docs_sdk}modules/index.adoc
:url_plugins: {docs_sdk}plugins/index.adoc

A reference of all actions and events that are provided by the Klayr Framework for a blockchain application.

TIP: For more information about the public application endpoints, check out the xref:{url_general_endpoints}[] page.

== Module- and plugin-specific actions and events

Modules and plugins often expose additional events and actions to the application.
All actions and events registered with the application are available as RPC endpoints.

The actions and events of the official modules and plugins are described on their respective reference pages, see xref:{url_modules}[] and xref:{url_plugins}[].
...
Actions are part of the public request / response API, and are invoked via RPCs.
