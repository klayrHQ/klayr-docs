...
:url_chain_getGeneratorList: api/klayr-node-rpc.adoc#chain_getgeneratorlist
...
:url_lip_70: https://github.com/KlayrHQ/lips/blob/350d08a90bdbedb3485363a40dd34f3ccadf0b7d/proposals/lip-0070.md
:url_constraints-on-commission-increase: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0070.md#constraints-on-commission-increase
:url_reward_distribution: https://github.com/KlayrHQ/lips/blob/main/proposals/lip-0070.md#distribution-of-rewards
...
In the Klayr protocol, validators receive rewards for generating blocks.
...
The reward-sharing mechanism is part of the PoS module. To learn more about how reward sharing works in Klayr Protocol, see {url_lip_70}[LIP 70^].
...
* To earn rewards, a *staker* must stake KLYs for an active block-generating validator.
...
=== Sharing via Klayr Desktop
To share rewards via Klayr Desktop, the validator should be logged in to their account and then perform the following steps:
...
+
. Klayr Desktop will display the *Transaction summary* including the existing and new value of commission.
...
. Klayr Desktop will display an *Edit commission successful* message after submitting the request successfully.
+
...
[source,bash]
----
klayr-core transaction:create pos changeCommission 134000 --json --pretty --send
----
--
Sidechain Node::
+
...
In this example, the validator is set to keep *25%* of the rewards and share *75%* with their stakers, hence the value of `2500`.
+
---- 
? Please enter passphrase:  [hidden]
? Please enter: newCommission:  2500
---- 
+
...
----
{
    ...
    "address": "klym87us5hykopm2f2nxa92z5ftbr9r52kg5b45e6",
    "punishmentPeriods": []
}
----
...
The part of rewards that is attributed to the validator that generated the block is automatically assigned to them by the Klayr Protocol.
...
On the other hand, stakers need to manually claim their rewards, and that can be done via both Klayr Desktop and CLI.
...
To claim rewards via Klayr Desktop, a staker must log in to their account on Klayr Desktop and perform the following steps to collect earned rewards:

. As soon as the staker earns a reward, they will be notified about it by Klayr Desktop.
...
+
. Klayr Desktop will display a *Transaction summary* to the staker.
If everything looks correct, click on the *Confirm* button.
+
...
+
. Upon successful request submission, Klayr Desktop will display a *Transaction submitted* successfully message.
+
...
[source,bash]
----
klayr-core transaction:create pos claimRewards 127000 --json --pretty --send
----
--
Sidechain Node::
+
