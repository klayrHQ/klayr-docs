...
:url_guides_setup_bootstrap: build-blockchain/create-blockchain-app.adoc#bootstrapping-a-new-blockchain-application-with-klayr-commander
:url_commander_commands: {docs_sdk}references/klayr-commander/commands.adoc
:url_framework_dashbobardplugin: {docs_sdk}plugins/dashboard-plugin.adoc
:url_elements: {docs_sdk}references/klayr-elements/index.adoc
:url_elements_crytpography: {docs_sdk}references/klayr-elements/cryptography.adoc
...
:url_protocol_block_forgers: understand-blockchain/klayr-protocol/blocks.adoc#forgers
:url_protocol_dpos: understand-blockchain/klayr-protocol/consensus-algorithm.adoc
:url_protocol_delegate_selection: understand-blockchain/klayr-protocol/consensus-algorithm.adoc#delegate_selection
:url_protocol_blocks_punishment: understand-blockchain/klayr-protocol/consensus-algorithm.adoc#punishment
:url_protocol_tx_registerdelegegate: understand-blockchain/klayr-protocol/transactions.adoc#delegate-registration
:url_sdk_protocol_puninshment: understand-blockchain/klayr-protocol/consensus-algorithm.adoc#punishment
:url_ref_cli: v3@klayr-core::reference/cli.adoc
:url_mgmt_accounts: v3@klayr-core::management/account-management.adoc#how-to-voteunvote-delegates
...
TIP: There are many possibilities to post the transaction, use the one you prefer: {url_klayr_wallet}[Klayr Desktop^], xref:{url_elements}[Klayr Elements], or the xref:{url_framework_dashbobardplugin}[].
...
[source,bash]
----
klayr-core transaction:create 5 0 1100000000 --asset='{"username":"myname"}'
...
[source,bash]
----
klayr-core account:get ed86183c22f8399f7aa28f7d2c2f1680224f7281 --pretty
----
--
Klayr app::
+
...
[source,bash]
----
cp ~/klayr-core/config/mainnet/config.json ~/klayr-core/config/mainnet/custom-config.json
----
--
Klayr app::
+
...
If the blockchain application was xref:{url_guides_setup_bootstrap}[bootstrapped with Klayr Commander], it is possible to conveniently generate the relevant forging data using the command xref:{url_references_cli_forgingconfig}[forging:config].
...
[source,bash]
----
$ klayr-core forging:config --output ./delegate_config.json
...
Please ensure that the xref:{url_commander_commands}[Klayr Commander] is installed in a secure environment.
Upon completion, please follow these commands to generate the encrypted passphrase:

[source,bash]
----
$ klayr
klayr passphrase:encrypt --json
...
The hash onion can be generated with Klayr Commander in the following manner:

[source,bash]
----
klayr hash-onion --json
----
...
For more information about the configuration of the Klayr SDK check out the xref:{url_guides_config}[configuration guide].
...
<2> Enable IPC to be able to use all commands of the Klayr Core CLI (see xref:{url_ref_cli}[])
...
To check the forging status of a Klayr Core node, execute the following command:
...
If the blockchain application was xref:{url_guides_setup_bootstrap}[bootstrapped with Klayr Commander], it is possible to conveniently generate the relevant forging data using the command xref:{url_references_cli_forgingenable}[forging:enable].
...
[source,bash]
----
klayr-core forging:enable 9bd82e637d306533b1e1ad66e19ca0047faa1a6a --use-status-values #<1>
----
--
Klayr app::
+
...
USAGE
  $ klayr-core forging:enable ADDRESS [HEIGHT] [MAXHEIGHTPREVIOUSLYFORGED] [MAXHEIGHTPREVOTED]
...
[source,js]
----
const { createWSClient } = require('@klayrhq/klayr-api-client');
...
Sometimes it is necessary to disable forging, for example to update to the latest Klayr Core version, or if you are moving your node to a different server.
...
* Alternatively, use {url_klayr_wallet}[Klayr Desktop^] to unvote.
====
...
[source,bash]
----
klayr-core forging:disable 9bd82e637d306533b1e1ad66e19ca0047faa1a6a #<1>
----
--
Klayr app::
+
...
USAGE
  $ klayr-core forging:disable ADDRESS
...
[source,bash]
----
klayr-core forger-info:import ./forger.db.tar.gz
----
. <<add-delegate-data-to-config>>.
...
Instructions::
. Start a new node with forging deactivated and synchronize with the Klayr blockchain until there is a block `finalizedBlock` that is
