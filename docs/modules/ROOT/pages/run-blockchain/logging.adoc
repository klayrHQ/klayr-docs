...
For monitoring or debugging your node, the xref:{url_klayr_sdk}[Klayr SDK] tracks all activities that occur within the node by creating applicable log messages.
...
By default, the generated log files are saved inside of the `logs` folder under `~/.klayr/APP_NAME/logs/klayr.log`.
...
It is recommended to set up some form of log rotation for the log files of Klayr SDK.
...
Now go to the logrotate config directory and create a new logrotate file for Klayr SDK as shown below:
