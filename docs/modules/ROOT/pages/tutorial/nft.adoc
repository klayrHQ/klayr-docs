...
:experimental:
// External URLs
:url_github_nft: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/tutorials/nft
:url_react_docs: https://reactjs.org/docs/getting-started.html
// Project URLs
:url_build_bapp: build-blockchain/create-blockchain-client.adoc#bootstrapping-a-new-blockchain-client-with-klayr-commander
...
// :url_references_token_module: klayr-sdk::modules/token-module.adoc
...
IMPORTANT: The example app for this tutorial was build without bootstrapping via Klayr Commander, therefore the project setup and file structure are different in this tutorial.
If you wish to see an example how to easily bootstrap a blockchain application with the Klayr Commander, check out xref:{url_build_bapp}[how to bootstrap the Hello World application].
...
* Digital tokens: Bitcoin(BTC) or Klayr(KLY).
a|
...
[source,js]
----
const { codec, cryptography } = require("klayr-sdk");

const registeredNFTTokensSchema = {
  $id: "klayr/nft/registeredTokens",
  type: "object",
...
* `stateStore`: The state store is a data structure that holds a temporary state while processing a block.
...
class PurchaseNFTAsset extends BaseAsset {
  ...
  async apply({ asset, stateStore, reducerHandler, transaction }) {
    ...
    // debit KLY tokens from purchaser account <5>
    await reducerHandler.invoke("token:debit", {
      address: purchaserAddress,
      amount: purchaseValue,
    });

    // credit KLY tokens to purchaser account <6>
...
We use the `reducerHandler` here and invoke `token:debit` from the token module which allows us to conveniently debit tokens from an account.
...
We use the `reducerHandler` here and invoke `token:debit` from the token module which allows us to conveniently credit tokens to an account.
...
The minimum value for it is `1000`, as the other IDs are reserved for future default modules of the Klayr SDK.
...
const { cryptography, codec, db } = require("klayr-sdk");
...
const getDBInstance = async (dataPath = '~/.klayr/nft-app/', dbName = 'nft_plugin.db') => {
  ...
  decodedBlock.payload.map(async trx => {
    ...
    if (module.name === 'nft') {
      ...
      if (trx.assetID === CREATENFT_ASSET_ID){
          ...
          for (let i = 0; i < val.length; i++) {
            ...
            if (val[i].ownerAddress === senderAdress.toString('hex')) {
              ...
              if (savedHistory && savedHistory.length < 1) {
                base32Address = cryptography.getBase32AddressFromPublicKey(Buffer.from(trx.senderPublicKey, 'hex'), 'kly');
        ...
        base32Address = (trx.assetID === TRANSFERNFT_ASSET_ID) ? cryptography.getBase32AddressFromAddress(Buffer.from(trx.asset.recipient, 'hex')) : cryptography.getBase32AddressFromPublicKey(Buffer.from(trx.senderPublicKey, 'hex'), 'kly');
        savedHistory = await getNFTHistory(db, dbKey);
        nftHistory = [Buffer.from(base32Address, 'binary'), ...savedHistory];
        encodedNFTHistory = codec.encode(encodedNFTHistorySchema, { nftHistory });
        await db.put(dbKey, encodedNFTHistory);
...
const { BasePlugin, codec } = require("klayr-sdk");
const pJSON = require("../package.json");
...
NOTE: When using the macOS, please ensure that a different port other than `5000` is used, for example, `8756` or `4724`, as port `5000` is already used by the latest macOS starting from v12 Monterey and above.
...
This tutorial is mainly about explaining how to build with the Klayr SDK, therefore other parts of the frontend app are not explained in much detail here.
...
For convenience, clone the `development` branch from the `klayr-sdk-examples` GitHub repository and use the prepared NFT frontend_app from the sdk examples.

.nft/
[source,bash]
----
git clone https://github.com/KlayrHQ/klayr-sdk-examples.git
mv klayr-sdk-examples/tutorials/nft/frontend_app frontend_app
...
----
15:20:56 INFO klayr-framework: Added transaction to pool (module=klayr:app)
{
...
15:21:00 INFO klayr-framework: New block added to the chain (module=klayr:app)
{
...
The transactions are created and signed by utilizing the `signTransaction()` from the `@klayrhq/klayr-client` package.
...
import { transactions, codec, cryptography } from "@klayrhq/klayr-client";
import { getFullAssetSchema, calcMinTxFee } from "../common";
import { fetchAccountInfo } from "../../api";

export const createNFTTokenSchema = {
  $id: "klayr/create-nft-asset",
  type: "object",
  required: ["minPurchaseMargin", "initValue", "name"],
  properties: {
    minPurchaseMargin: {
...
export const createNFTToken = async ({
  ...
  const { id, ...rest } = transactions.signTransaction(
    createNFTTokenSchema,
    {
      ...
      asset: {
        name,
        initValue: BigInt(transactions.convertKLYToBeddows(initValue)),
  ...
  const { id, ...rest } = transactions.signTransaction(
    purchaseNFTTokenSchema,
    {
      ...
      asset: {
        ...
        purchaseValue: BigInt(transactions.convertKLYToBeddows(purchaseValue)),
      },
    },
    Buffer.from(networkIdentifier, "hex"),
    passphrase
...
To create the account details, the `passphrase` and `cryptography` library of the `@klayrhq/klayr-client` package are used.
...
import * as api from "../../api";
import { transactions } from "@klayrhq/klayr-client";
...
export default function PurchaseNFTTokenDialog(props) {
  ...
  const currentValue = parseFloat(
    transactions.convertBeddowsToKLY(props.token.value)
  );
...
====
As you may be aware, the account address can be represented in different formats, such as `bytes`, `Buffer`,`hex` or `Klayr32`.

The `Klayr32` format is the most human-readable representation of an account address, and therefore users of the application should only see this address in the UI.
...
import { transactions, cryptography, Buffer } from "@klayrhq/klayr-client";
...
export default function NFTToken(props) {
  ...
  const base32UIAddress = cryptography.getBase32AddressFromAddress(Buffer.from(props.item.ownerAddress, 'hex'), 'kly').toString('binary');
  return (
    <Card>
      <CardContent>
        ...
        <dl className={classes.propertyList}>
          ...
          <li>
            <dt>Token value</dt>
            <dd>{transactions.convertBeddowsToKLY(props.item.value)}</dd>
          </li>
          <li>
