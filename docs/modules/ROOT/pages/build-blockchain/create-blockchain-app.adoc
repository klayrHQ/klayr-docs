...
:url_github_guides_bootstrap: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/guides/01-bootstrap/hello_app
// Project URLs
...
How to create a new blockchain application with the Klayr SDK.

[TIP]

====
To get started with the development of a blockchain application in the most simple and efficient manner, it is *recommended* to follow the guide <<bootstrapping-a-new-blockchain-application-with-klayr.xyzmander>>.

If you prefer not to use Klayr Commander to get started with development, the <<manual-setup-alternative-without-klayr.xyzmander>> is explained below.
====

== Bootstrapping a new blockchain application with Klayr Commander

Klayr Commander offers various commands related to <<initializing-a-new-blockchain-application>> which can be used to conveniently get started with developing a blockchain application.
...
The Klayr SDK will run on both the Ubuntu and MacOS operating systems shown below:
...
It is recommended to install Klayr Commander globally with the NPM (Node Packet Manager), in order to facilitate the convenient usage of the Klayr Commander CLI.
...
----
$ klayr --version
klayr.xyzmander/5.1.10 darwin-x64 node-v16.15.0
----
...
Inside of this folder, execute the `init` command of Klayr Commander as shown below:
...
The command `klayr generate:module` creates new modules in this folder.
<6> `modules.ts` Registers the modules with the application.
<7> `plugins/`: Contains internal plugins of the application.
This folder is empty after the first initialization with `klayr init`.
The command `klayr generate:plugin` creates new modules in this folder.
...
These files create a ready-to-start blockchain application configured for a local devnet, which uses only the default modules of the Klayr SDK.
...
[source,typescript]
----
import { Application, PartialApplicationConfig, utils } from 'klayr-sdk';
...
After the application was started for the first time, the corresponding application data can be found under the path `~/.klayr/hello_app/`
...
Once the application is started for the first time, it will save the application-specific data under the path `~/.klayr/hello_app/`.

To reset the database of the application, simply delete the folder with the application data:

[source,bash]
----
rm -r ~/.klayr/hello_app/data/
----

=== Next steps

By installing Klayr Commander and running `klayr init`, a working blockchain application now exists with the default configurations for running in a local devnet.
...
== Manual Setup (alternative without Klayr Commander)

How to create a new blockchain application manually without using the Klayr Commander.
...
We want to use this file to store the code that will start the blockchain application by using the Klayr SDK.

In `index.js`, import the `Application`, `genesisBlockDevnet`, and `configDevnet` from the the `klayr-sdk` package.
...
[NOTE]
====
The `klayr-sdk` package contains the sample objects `genesisBlockDevnet` and `configDevnet` which enable the user to quickly spin up a development blockchain network.
...
[NOTE]
====
After completing these steps, the default blockchain application of the Klayr SDK will now be running.
