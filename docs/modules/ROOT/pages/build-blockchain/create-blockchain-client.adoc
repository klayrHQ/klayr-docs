...
.Output
----
klayr-commander/6.1.2 darwin-arm64 node-v18.20.4
----
...
IMPORTANT: When choosing a chainID it needs to start with it's mainchain byte `00` for mainnet, `01` for testnet, `02` for betanet and `04` for devnet. The chainID can be changed later in the `config.json` file.
...
? Chain ID in hex representation. ChainID must be 4 bytes (8 characters) 01234567
