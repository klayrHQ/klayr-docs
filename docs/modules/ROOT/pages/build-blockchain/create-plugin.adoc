...
// URLs
:url_github_guides_plugin: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/guides/04-plugin/hello_app
// Project URLS
...
:url_references_commander_commands_plugin: {docs_sdk}references/klayr.xyzmander/commands.adoc#generate-plugin
...
[NOTE]
====
View the complete sample code of this guide on GitHub in the {url_github_guides_plugin}[Klayr SDK examples repository^].
====
...
While in the root folder of your blockchain application, generate a skeleton for the new plugin with Klayr Commander.
...
For a complete overview of all available options of the `generate:plugin` command, visit the xref:{url_references_commander_commands_plugin}[Klayr Commander command reference].

[[generate-plugin]]
[source,bash]
----
klayr generate:plugin latestHello
----
...
Registering plugin...
 conflict .klayrrc.json
? Overwrite .klayrrc.json? overwrite
    force .klayrrc.json
...
[source,typescript]
----
import { BasePlugin, PluginInfo } from 'klayr-sdk';
import type { BaseChannel, EventsDefinition, ActionsDefinition, SchemaWithDefault } from 'klayr-sdk';
...
The plugin class always extends from the `BasePlugin`, which is imported from the `klayr-sdk` package.
