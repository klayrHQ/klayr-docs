...
// URLs
:url_github_guides_module: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/guides/02-module/hello_app
...
:url_rpc_endpoints: api/klayr-node-rpc.adoc#actions
:url_references_schemas: understand-blockchain/codec-schema.adoc
:url_references_commander_commands_module: {docs_sdk}references/klayr.xyzmander/commands.adoc#generate-module
...
[NOTE]
====
View the complete sample code of this guide on GitHub in the {url_github_guides_module}[Klayr SDK examples repository^].
====
...
* Klayr Commander is installed, and a basic blockchain application is already initialized, as explained in the previous guide xref:{url_guides_setup}[].
====

== Generating the module skeleton

While in the root folder of your blockchain application, generate a skeleton for the new module with Klayr Commander.

The command `klayr generate:module` expects 2 arguments:
...
For a complete overview of all available options of the `generate:module` command, visit the xref:{url_references_commander_commands_module}[Klayr Commander command reference].
...
[source,bash]
----
klayr generate:module hello 1000
----
