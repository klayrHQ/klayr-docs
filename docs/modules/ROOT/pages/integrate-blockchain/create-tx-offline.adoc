...
* `@klayrhq/klayr-client`, including the following libraries:
** {url_typedoc_transactions}[@klayrhq/klayr-transactions^]
** {url_typedoc_validator}[@klayrhq/klayr-validator^] (optional)
...
It is recommended to verify the correct format of the transaction with the `validator.validate()` function of the `@klayrhq/klayr-validator` package afterwards.
...
[source,js]
----
const { validator } = require('@klayrhq/klayr-client');
...
To sign the transaction, use the `signTransaction()` function of the `@klayrhq/klayr-transactions` package.
