...
=== Creating transactions with klayr-client

==== Require the transaction and asset schema

To create, validate, and sign transactions offline with the klayr-client, it is necessary to access their schemas.
...
[source,typescript]
----
const { validator, transactions } = require('@klayrhq/klayr-client');
...
const account = {
  ...
  "address": "klyuwzrd73pc8z4jnj4sgwgjrjnagnf8nhrovbwdn"
};
...
if (transactionErrors.length) {
  throw new validator.KlayrValidationError([...transactionErrors]);
}
...
IMPORTANT: xref:{url_service}[Klayr Service] needs to be installed in order to post a transaction via the Klayr Service HTTP API.

cURL is one of the tools that can be used to send HTTP API requests to Klayr Service as shown below:
...
cURL is one of the tools that can be used to send HTTP API requests to Klayr Core as shown below:
