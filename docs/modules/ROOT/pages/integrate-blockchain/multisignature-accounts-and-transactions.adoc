...
:toclevels: 6
// URLs
:url_typedoc_crypto: {site-url}/klayr-sdk/v6/references/typedoc/modules/_klayrhq_klayr_cryptography.html
:url_klayr_learn_create_account: https://klayr.xyz/learn/undefined/create-an-account
:url_github_guides_multisig: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/guides/tx-creation-and-signing/multisig.js
// Project URLs
:url_sdk_commander: klayr-sdk::references/klayr.xyzmander/index.adoc
...
:url_accounts: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/guides/register-multi-sig-accounts/accounts.json
:url_account_registration_script: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/guides/register-multi-sig-accounts/register-multiSig-account.js
:url_multiSig_reg_directory: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/guides/register-multi-sig-accounts
:url_multiSigtx_creation_signing_directory: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/guides/multiSigtx-creation-signing
...
+
--
For a step-by-step guide about how to create a new account with Klayr Desktop, please refer to: {url_klayr_learn_create_account}[Create an Account^].

--
Klayr SDK::
+
--
For a step-by-step guide on creating a new account with Klayr SDK, please refer to the xref:{url_integrate_accounts}[] guide.
...
----
{
   "accounts": [
      {
         "address": "kly6oqambp2mgqvqxxxm5aohs6s6329sgwftax2rf",
         "keyPath": "m/44'/134'/0'",
      ...
      {
         "address": "klygmmkgyc67n5jrpf2trgtxtc4yjg7bpu992chba",
         "keyPath": "m/44'/134'/1'",
      ...
      {
         "address": "klyrw8gmg6fgdkkgj48q364quzkoqzya479x7rc3d",
         "keyPath": "m/44'/134'/2'",
...
[source,js]
----
const { apiClient, cryptography, codec, Transaction, multisigRegMsgSchema } = require('klayr-sdk');
const { ed, address } = cryptography;
const { accounts } = require('./accounts.json');
const { registerMultisignatureParamsSchema, } = require('klayr-framework/dist-node/modules/auth/schemas');
----
+
...
----
    const { nonce } = await appClient.invoke('auth_getAuthAccount', {
        address: address.getKlayr32AddressFromPublicKey(Buffer.from(senderKeyInfo.publicKey, 'hex')),
    });

    const unSignedData = {
        address: address.getAddressFromKlayr32Address(senderKeyInfo.address),
        nonce: BigInt(nonce),
        numberOfSignatures: 2,
...
----
klayr-core transaction:create auth registerMultisignature 100000000 --json --pretty
----
...
+
--
When the member receives the JSON, they can use Klayr Desktop to sign the JSON like so:
...
To add a signature to a transaction, use the `ed.signDataWithPrivateKey()` method of the {url_typedoc_crypto}[klayr-cryptography^] package.
This package is also part of the `klayr-sdk` package.
...
// Don't forget to specify the other required parameters for `ed.signDataWithPrivateKey()`, especially `chainID` should be the correct chain ID for the account, and the `tag` for multi-signature registrations should always be `KLY_RMSG_`.

// An example script can be found in the klayr-sdk-examples repository: {url_github_guides_multisig}[multisig.js^]
...
Log in to Klayr Desktop with your account that is defined as part of the new multi-sig group.

Copy the transaction from above in JSON string format and proceed to sign the transaction object in Klayr Desktop, as explained in <<collecting-signatures>>.
////
...
// For a step-by-step guide on sending a transaction with Klayr SDK, please refer to the xref:{url_integrate_posttxs_apiclient}[How to post a transaction] guide.
--
=====
...
+
--
If you used Klayr Desktop to collect the required signatures, it is possible to directly send it from Klayr Desktop, after the last required signature was added to the transaction.

In case you want to send the transaction via CLI, download or copy the JSON from Klayr Desktop and send it like this:

Use the console of xref:{url_core}[Klayr Core] or xref:{url_sdk_commander}[Klayr Commander] to encode the transaction JSON;
...
----
klayr-core > let hexTx
klayr-core > api.then((client) => {
...
----
klayr-core transaction:send 
----
////
...
For Klayr Desktop, it is important to note that the transaction object can only be created from the multi-signature account itself (not its other members).
...
+
--
In Klayr Desktop, it is only possible to create a multi-signature transaction by the account owner of the multi-sig account, i.e. the person owning the passphrase/private key of the account.
...
For this, please refer to the examples in the "Node CLI" and "Klayr SDK" tabs.

Prepare a transaction, for example, a token transfer::
From Klayr Desktop, click on the kbd:[Send] button as you would normally to create a token transfer.
...
----
klayr-core transaction:create token transfer 1000000 --no-signature --sender-public-key e98e8a6325730be6bf2644af83d5a0b004bb31c15858fedbd0ac2c1f89e2eece
----
...
A sample script is available in the klayr-sdk-examples repository where a token transfer transaction is first created and signed by a signatory.
...
[,js]
----
const { apiClient, cryptography, codec, Transaction, transactions } = require('klayr-sdk');
const { transferParamsSchema } = require('klayr-framework/dist-node/modules/token/schemas');
const { accounts } = require('./accounts.json');
...
const tokenTransferParams = {
    tokenID: tokenID,
    recipientAddress: cryptography.address.getAddressFromKlayr32Address(accounts[1].address),
...
In Klayr Desktop, click on the kbd:[Sign multisignature] button in the left menu.
...
----
klayr-core transaction:sign 0a05746f6b656e12087472616e73666572180120c0843d2a20e98e8a6325730be6bf2644af83d5a0b004bb31c15858fedbd0ac2c1f89e2eece32270a0800000001000000001080dac4091a14f94b4fc46a71d7c913d89cbf30cc698f3ee3120d2200 --optional-keys c61cd862a8b7f73857b248a4358a7b35c29ca273d76ba3819e8c54b62801f16e e98e8a6325730be6bf2644af83d5a0b004bb31c15858fedbd0ac2c1f89e2eece --json --pretty
----
...
----
klayr-core transaction:sign  --optional-keys c61cd862a8b7f73857b248a4358a7b35c29ca273d76ba3819e8c54b62801f16e e98e8a6325730be6bf2644af83d5a0b004bb31c15858fedbd0ac2c1f89e2eece --json --pretty
----
