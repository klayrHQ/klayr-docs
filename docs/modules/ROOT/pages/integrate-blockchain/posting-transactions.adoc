...
:url_sdk_cli_txsend: klayr-sdk::client-cli.adoc#transactionsend
...
The Klayr API client is a JS package that allows connecting to a node API through WS and IPC protocols.
...
//TODO: Comment back in and update once Klayr Service v0.7 docs are updated
////
== Via Klayr Service WS API

If you prefer to use the RPC WebSocket API of Klayr Service to post transactions, this can be achieved for example by writing a small JS script, using the `socket.io-client` package.
The `transaction` is expected as parameter in the xref:{url_integrate_createtx_hex}[hex string format].
...
== HTTP Klayr Service API example with cURL
