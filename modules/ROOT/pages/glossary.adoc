...
:url_fork: https://klayr.com/blog/research/klayr-mainnet-regenesis

:url_delegate_misbehavior: understand-blockchain/klayr-protocol/transactions.adoc#pom
:url_discover_misbehavior: 6.0.0@klayr-sdk::reference/klayr-framework/dpos-module.adoc#discover
:url_dpos: 6.0.0@klayr-sdk::reference/klayr-framework/dpos-module.adoc#dpos
:url_delegate_registration: 6.0.0@klayr-sdk::reference/klayr-framework/dpos-module.adoc#delegate_registration
:url_delegate_registration_tx: understand-blockchain/klayr-protocol/transactions.adoc#delegate

:url_genesis_block: understand-blockchain/klayr-protocol/blocks.adoc#genesis-block
:url_reclaim_kly: klayr-core::index.adoc#kly-reclaim
:url_tx_properties: understand-blockchain/klayr-protocol/transactions.adoc#transaction-properties
:url_multisignature: understand-blockchain/klayr-protocol/transactions.adoc#multisignature
:url_token_transfer: understand-blockchain/klayr-protocol/transactions.adoc#transfer
:url_token_unlock: understand-blockchain/klayr-protocol/transactions.adoc#unlock
...
Addresses are unique identifiers for accounts in the Klayr blockchain.
They are the 20-byte hash value of the public key that controls the account initially.
In all front-end products, addresses are displayed in a human-readable representation called <<klayr32-representation>>.
An example for an address in Klayr32 presentation is `klyxwnb4ubt93gz49w3of855yy9uzntddyndahm6s`.
When referring to addresses in Klayr, we always assume the Klayr32 representation, without explicitly having to mention it.
In the Klayr protocol and SDK only the 20 byte value is stored for efficiency and sometimes the hexadecimal representation of the address is used.
...
An active delegate can mostly be considered as a forging delegate, who is able to forge new blocks for the Klayr blockchain. Rank 1-101.
...
A Beddow is the smallest unit of a KLY token. 100 million Beddows is equal to 1 <<KLY>>.

=== Betanet

A temporary, public testing network for the Klayr Core beta candidate.
...
A xref:{url_blockchain_app}[blockchain application] is any kind of application which uses its own blockchain as a database layer. For example, this can be an application running on its own blockchain built with the Klayr SDK.
...
The Klayr SDK provides dedicated genesis block and configurations to conveniently set up a local Devnet during development of a blockchain application.
...
It is part of the consensus algorithm of a blockchain and used on Klayr Mainnet and in the Klayr SDK.
...
A Faucet is generally defined in the blockchain world as a tool whereby users can receive a certain amount of tokens for free. For Klayr blockchain apps, the faucet plugin can be registered with the application to provide a basic faucet, which is useful during the development phase, and also for proof-of-concept blockchain applications.

=== Forging

How delegates produce new blocks on the Klayr blockchain.
...
For further detailed information regarding forking in the Klayr network, see the following {url_fork}[Klayr Mainnet hard fork process^] description in the Klayr Mainnet Regenesis page

=== Framework Plugin

A self-contained, off chain component which extends the standard feature set of the Klayr SDK by a specific use case, e.g. a node monitor plugin or delegate forging plugin.
...
The hash onion stores the random seeds of the delegate and is required for each forging round, this can be generated by the Klayr Commander.
...
Klayr Framework is an application framework responsible for establishing and maintaining the interactions between the modules of a blockchain application compatible with the Klayr protocol. It provides an application where off-chain and on-chain logic can be extended with user defined plugins and modules.

=== LIP - Klayr Improvement Proposal

A LIP is a document usually structured by the research team defining technical changes in the Klayr protocol.
In addition, it can also cover an implementation or a non technical process surrounding Klayr.
Furthermore, a LIP also describes the requirements, rationale and motivation for the required changes.

=== Klayr SDK

The Klayr SDK is an open-source software development kit which enables developers to easily build scalable Blockchain applications using JavaScript.

=== Klayr32 representation

The Klayr32 representation of an <<account-address>> starts with the letters "kly" followed by a custom Base32 encoding of the 20 bytes of the address appended with a BCH checksum.
...
The choice of the Klayr32 representation of addresses does not modify the address, but is introduced to provide a safer and more pleasant experience to users.

=== KLY

The token used in the Klayr Mainnet.

== M

=== Mainnet

A xref:{url_mainnet}[mainnet] is a fully developed functional and independent blockchain, running on its own network and using its own protocol and technology. It is the official main network within Klayr.
...
A transaction type on all networks based on the Klayr protocol which registers a xref:{url_multisignature}[multisignature group].
...
A blockchain application built with the Klayr SDK, e.g. Klayr Core, which is connected to other nodes.
...
In the Klayr network it can be defined as an integer that represents for the number of outgoing transactions of an account.
...
=== Reclaim KLY transaction

This transaction is only part of Klayr Core and not the Klayr SDK.
A xref:{url_reclaim_kly}[reclaim KLY transaction] type on the Mainnet converts a previously non-initialized legacy account’s address to the new address system.
...
The official testing network of Klayr. It can be thought of as an instance of a blockchain which is used for testing purposes, and consists of a the same version of the mainnnet underlying software. All testing and experimentation can be performed on the testnet without jeopardizing the actual mainnet.

=== Token

A token refers to KLY or any sidechain token. A  digital token is designated to a cryptocurrency that is built on top of an existing blockchain, and represents a particular tradable and fungible asset.

=== Transaction

An interaction with the Klayr blockchain which permanently writes data to it.
...
All properties of a transaction object are described on the xref:{url_tx_properties}[Klayr protocol > Transactions] page. The input data for the different <<module-asset, module assets>> is stored inside the <<transaction-asset>> of a transaction.
...
An amount of KLY needed to send a transaction on the Klayr blockchain.
...
An account deploying their KLY to vote for their favourite delegates.
...
A Klayr wallet is available on both Klayr Desktop and Klayr Mobile.
They are actively maintained and provide the most secure and easiest way to access the users KLY tokens.
