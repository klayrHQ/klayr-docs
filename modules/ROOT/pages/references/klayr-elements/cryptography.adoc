...
:page-aliases: klayr-elements/packages/cryptography.adoc, reference/klayr-elements/packages/cryptography.adoc
:toc:
:toclevels: 2
...
:url_guides_forging: {docs_general}run-blockchain/forging.adoc
:url_pbk_glossary: {docs_general}glossary.adoc#pbkdf2-password-based-key-derivation-function-2


The @klayrhq/klayr-cryptography package provides all the cryptographic functionality necessary when interacting with the Klayr ecosystem.
...
[source,bash]
----
npm install @klayrhq/klayr-cryptography
----
...
[source,bash]
----
npm update @klayrhq/klayr-cryptography
----

== Usage

The `klayr-cryptography` package can be used on both server-side and client-side code.

In case the `klayr-cryptography` is used server-side, it is beneficial to speed up the application by using the {url_github_sodium}[sodium-native^] library.
...
----
{
  SIGNED_MESSAGE_PREFIX: 'Klayr Signed Message:\n',
  BINARY_ADDRESS_LENGTH: 20,
  DEFAULT_KLAYR32_ADDRESS_PREFIX: 'kly',
  DEFAULT_BASE32_ADDRESS_PREFIX: 'kly'
}
----
...
[source,js]
----
import * as cryptography from '@klayrhq/klayr-cryptography';
...
[source,js]
----
const networkIdentifier = klayr.cryptography.getNetworkIdentifier(
  klayr.cryptography.hexToBuffer("23ce0366ef0a14a91e5fd4b1591fc880ffbef9d988ff8bebf8f3666b0c09597d"),
  "Klayr",
...
This returns the Klayr address for a provided passphrase.
...
This converts a public key into a Klayr address.
...
`Buffer`: This is the Klayr address for the public key.
...
=== verifyChecksum
To verify the checksum of Klayr32 addresses.
...
Converts the public key input to a Klayr32 address.

==== Syntax

[source,js]
----
getKlayr32AddressFromPublicKey(publicKey, prefix)
----

==== Parameters

. `publicKey`(Buffer): The public key which is used to retrieve the Klayr32 address.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `kly`.

==== Return value

`string`: The corresponding address in Klayr32 format.

=== getKlayr32AddressFromPassphrase

Converts the passphrase input to a Klayr32 address.

==== Syntax

[source,js]
----
getKlayr32AddressFromPassphrase(passphrase, prefix)
----

==== Parameters

. `passphrase`(string): The passphrase which is used to retrieve the Klayr32 address.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `kly`.
...
Returns an account address as a Buffer for a given Klayr32 address.

==== Syntax

[source,js]
----
getAddressFromKlayr32Address(Klayr32Address,prefix)
----

==== Parameters

. `Klayr32Address`(string): A Klayr32 address.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `kly`.
...
[source,js]
----
getKlayr32AddressFromAddress(address,prefix)
----
...
`string`: The corresponding Klayr32 address.

=== validateKlayr32Address

Validates if the input address is a correct Klayr32 address or not.

==== Syntax

[source,js]
----
validateKlayr32Address(address, prefix)
----

==== Parameters

. `address`(string): An account address in Klayr32 format.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `kly`.
...
. `passphrase`: The secret passphrase used to sign the message.
...
[source,js]
----
const stringToPrint = cryptography.signAndPrintMessage('Hello Klayr!',  'robust swift grocery peasant forget share enable convince deputy road keep cheap');
console.log(stringToPrint);
// -----BEGIN KLAYR SIGNED MESSAGE-----
//-----MESSAGE-----
//Hello Klayr!
...
[source,js]
----
cryptography.signMessageWithPassphrase('Hello Klayr!',  'robust swift grocery peasant forget share enable convince deputy road keep cheap');
/* {
  message: 'Hello Klayr!',
  publicKey: <Buffer 9d 30 58 17 5a ca b9 69 f4 1a d9 b8 6f 7a 29 26 c7 42 58 67 0f e5 6b 37 c4 29 c0 1f ca 9f 2f 0f>,
...
[source,js]
----
const msg = cryptography.signMessageWithPassphrase('Hello Klayr!',  'robust swift grocery peasant forget share enable convince deputy road keep cheap');
