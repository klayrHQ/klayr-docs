...
The @klayr/cryptography package provides all the cryptographic functionality necessary when interacting with the Klayr ecosystem.
...
[source,bash]
----
npm install @klayr/cryptography
----
...
[source,bash]
----
npm update @klayr/cryptography
----

== Usage

The `cryptography` package can be used on both server-side and client-side code.

In case the `cryptography` is used server-side, it is beneficial to speed up the application by using the {url_github_sodium}[sodium-native^] library.
