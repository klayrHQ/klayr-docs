...
// Settings
:description: All available commands of Klayr Commander consisting of all available options, a usage example, and an example response are covered here.
:toc:
:idseparator: -
:idprefix:
:docs_general: klayr-v3@ROOT::
// Project URLs
:url_guides_config_genesis_block: {docs_general}build-blockchain/create-genesis-block.adoc
:url_aes_glossary: {docs_general}glossary.adoc#aes-256-cbc-advanced-encryption-standard
...
Bootstrap a blockchain application using the Klayr SDK.
...
Displays the Network identifier based on the payload hash of the xref:{url_guides_config_genesis_block}[genesis block] of the network.
...
It displays the Klayr address, the address as hexadecimal string, the publickey and the privatekey that belong to the entered passphrase:
...
This command uses the Klayr Elements passphrase module to encrypt a message you provide for a given public key using a randomly generated nonce.
...
This command verifies a message after being signed with the sign message command.
