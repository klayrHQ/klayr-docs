...
The {url_pm2_ecosystem_file}[PM2 ecosystem file^] provides the possibility to configure behavior, options, environment variables, and the log files of Klayr Service with PM2.
The default configuration file is `ecosystem.core3.config.js`, and is located in the root folder of `klayr-service`:
It contains a configuration to connect to a local Klayr Core node.

This config will be used by default when starting Klayr Service with the following:
	...
	apps: [
		...
		},
		{
			...
			env: {
				// --- Remember to set the properties below
				SERVICE_BROKER: 'redis://localhost:6379/0',
				KLAYR_CORE_WS: 'ws://localhost:8080',
				SERVICE_CORE_REDIS: 'redis://localhost:6379/1',
				SERVICE_CORE_REDIS_VOLATILE: 'redis://localhost:6379/8',
				SERVICE_CORE_MYSQL: 'mysql://root:password@localhost:3306/klayr',
		...
		},
		{
			...
			env: {
				SERVICE_BROKER: 'redis://localhost:6379/0',
				SERVICE_NEWSFEED_MYSQL: 'mysql://klayr:password@localhost:3306/klayr?charset=utf8mb4',
			},
		},
	],
};
...
To start Klayr Service with the custom ecosystem file, execute the following command:
...
Finally, if you want to use `npm start` and `npm stop` to start and stop Klayr Service with the custom configuration, don't forget to update the scripts in `package.json`:
