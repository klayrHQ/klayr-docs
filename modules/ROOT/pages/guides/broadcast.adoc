...
:toc:
:sectnums:
:url_klayr_wallet: https://klayr.io/wallet
...
:url_commander_commands_tx_create: reference/klayr-commander/commands.adoc#create-tx
:url_commander_commands_tx_broadcast: reference/klayr-commander/commands.adoc#broadcast-tx
...
To achieve this a small script in Javascript needs to be written, that utilizes xref:{url_elements_packages}[Klayr Elements packages] to create the transaction object, sign it, and send it to the network.

TIP: For the default transaction types, it is also possible to use Klayr Commander to xref:{url_commander_commands_tx_create}[create] and xref:{url_commander_commands_tx_broadcast}[broadcast transactions from the command-line] or {url_klayr_wallet}[Klayr Desktop^] to send a transaction via a user-friendly UI.
...
# v12.15.0
npm init --yes
npm i @klayrhq/klayr-transactions
npm i @klayrhq/klayr-cryptography
npm i @klayrhq/klayr-api-client
----
...
+
--
.How to create a tx object with Klayr Elements
[source,js]
----
...
.How to create a custom tx object with Klayr Elements
[source,js]
----
...
+
--
.How to sign tx object with Klayr Elements
[source,js]
----
const transactions = require('@klayrhq/klayr-transactions');
const {getNetworkIdentifier} = require('@klayrhq/klayr-cryptography');
...
.How to sign a custom tx object with Klayr Elements
[source,js]
----
const HelloTransaction = require('../hello_transaction');
const {getNetworkIdentifier} = require('@klayrhq/klayr-cryptography');
