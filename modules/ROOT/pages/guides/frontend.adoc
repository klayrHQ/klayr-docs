...
:description: This guide covers how to create a frontend or client app that interacts with a Klayr blockchain network.
:toc:
...
This guide describes how to create a frontend or client app that interacts with a Klayr blockchain network.

NOTE: A **Klayr blockchain network** is a network of nodes that are built with the {url_github_sdk}[Klayr SDK^].

== Choose a framework

Choose a framework based on personal preference.

NOTE: Klayr uses React.js as a framework of choice for the examples provided in the documentation.
...
Firstly, please  ensure you have all xref:{url_setup}[Klayr SDK dependencies] installed, and have a fresh database `klayr_dev`.

include::partial$dependencies.adoc[]

Clone the {url_github_sdk_examples}[klayr-sdk-examples in GitHub^] and navigate into `hello_world` folder.
...
[source,js]
----
import { APIClient } from '@klayrhq/klayr-api-client';
...
import { Mnemonic } from '@klayrhq/klayr-passphrase';
...
import{ TransferTransaction, utils } from '@klayrhq/klayr-transactions';
import * as cryptography from '@klayrhq/klayr-cryptography';
    ...
    handleSubmit = (event) => {
        ...
        const fundTransaction = new TransferTransaction({
            asset: {
                ...
                amount: utils.convertKLYToBeddows(this.state.amount),
            },
...
import {
    HelloTransaction,
} from 'klayr-hello-transactions';
import { api } from '../api.js';
import * as cryptography from '@klayrhq/klayr-cryptography';
import {utils} from "@klayrhq/klayr-transactions";
