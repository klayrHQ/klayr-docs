...
:description: The Klayr SDK Configuration page describes how to configure a blockchain application developed with the Klayr SDK.
:toc:
:v_sdk: v3.0.2

:url_github_config_devnet: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/sdk/src/samples/config_devnet.json
:url_github_constants_schema: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/framework/src/application/schema/constants_schema.js
:url_github_genesis_block: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/sdk/src/samples/genesis_block_devnet.json
:url_github_sdk: https://github.com/KlayrHQ/klayr-sdk/tree/{v_sdk}

:url_reference_config: reference/config.adoc

[[intro]]
The `Application` instance which is exported by the Klayr SDK, is configured to run with the <<config_options, default config options>> that are defined in the Klayr Framework.

To change the configuration, pass a config object as parameter when creating a new `Application` instance.

During development of a blockchain application it is recommended to import and use the {url_github_config_devnet}[configDevnet^] object from the Klayr SDK.
...
The **default** configuration options are located in the {url_github_sdk}[Klayr SDK repository^] in GitHub:
...
In the alpha version of the Klayr SDK, at present not all of the available constants are configurable by the user yet.
...
The Klayr SDK exposes an object `genesisBlockDevnet` that holds all of the required important information to spin up a local development network.

TIP: Go to Github, to see the full file {url_github_genesis_block}[genesis_block_devnet.json^]

[NOTE]
====
A genesis block generator to create genesis blocks will be included in the Klayr SDK eventually.
For Klayr Alpha SDK, the exposed `genesisBlockDevnet` can be used as a template, and customized to your requirements.
====
