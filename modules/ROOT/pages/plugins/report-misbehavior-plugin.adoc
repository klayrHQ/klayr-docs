...
`@klayrhq/klayr-framework-report-misbehavior-plugin` is a plugin for the xref:{url_klayr_framework}[Klayr Framework] that provides automatic detection of delegate misbehavior and sends a `reportDelegateMisbehaviorTransaction` to the running node.
