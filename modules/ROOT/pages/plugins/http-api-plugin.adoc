...
:url_guides_config: {docs_general}build-blockchain/configure-app.adoc
...
:url_utf_glossary: {docs_general}glossary.adoc#utf-8-unicode-transformation-format-8
:url_total_votes: {docs_general}glossary.adoc#total-votes-received

`@klayrhq/klayr-framework-http-api-plugin` is a plugin for the Klayr Framework that provides basic HTTP API endpoints to get running node information.
...
Click on an endpoint to view the descriptions, details and examples.
...
==== Contact information
[%hardbreaks]
__Contact Email__ : admin@klayr.com
...
|**seedReveal** +
__required__|Hex encoded value that contains the new seed value revealed after each round by the forging delegate.|string
|===
...
|**timestamp** +
__required__|Unix timestamp of block created in seconds.|number
|**transactionRoot** +
__required__|Hex encoded value of Merkle tree root of the transaction IDs.|string
|**version** +
__required__|Versioning for future upgrades of the Klayr protocol.|number
|===
...
**Example** : `"9d0149b0962d44bfc08a9f64d5afceb6281d7fb5"`|string
|**nextForgingTime** +
__required__|Returns the unix timestamp in seconds in which the forger will be able to forge the next block.
Each slot has a timespan of 10 seconds. +
**Example** : `4368793.0`|number
|**totalVotesReceived** +
__optional__|The  xref:{url_total_votes}[total votes received] by the delegate.
...
|Name|Description|Schema
|**accounts** +
__required__|Array of initial forging delegate accounts.|< <<_account,Account>> > array
|**initDelegates** +
...
|**networkVersion** +
__required__|The protocol version of Klayr Core that the peer node runs on. +
**Example** : `"1.1"`|string
|**syncing** +
...
**Example** : `"Klayr"`|string
|**delegateListRoundOffset** +
__required__|Number of previous rounds in which the list of delegates will be used for the current round. +
**Example** : `2`|integer
|**maxPayloadLength** +
...
**Example** : `2`|integer
|**totalAmount** +
__optional__|Total amount of KLY available in the network before the rewards milestone started. +
...
|**offset** +
__required__|Height in which the reward distribution starts. +
**Minimum value** : `1` +
...
**Example** : `"1.1"`|string
|**options** +
__required__|Optional field defined by the application.|object
|**port** +
...
|**assetID** +
__optional__|Asset Id for the transaction asset.|number
|**fee** +
...
|**moduleID** +
__optional__|Module Id for the transaction asset.|number
|**nonce** +
...
__required__|Object stored per transaction type.|object
|**assetID** +
__required__|Asset Id for the transaction asset.|number
|**fee** +
__required__|Transaction fee associated with this transaction.|string
|**moduleID** +
__required__|Module Id for the transaction asset.|number
|**nonce** +
__required__|Unique sequence of number per account. +
**Example** : `"1"`|string
|**senderPublicKey** +
