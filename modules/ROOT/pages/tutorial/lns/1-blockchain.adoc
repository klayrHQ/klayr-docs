...
:url_reference_dashboard: 6.0.0@klayr-sdk::plugins/dashboard-plugin.adoc
:url_reference_faucet: 6.0.0@klayr-sdk::plugins/faucet-plugin.adoc
:url_transaction_asset: glossary.adoc#transaction-asset
:sectnums:

== Dependencies

Please refer to the section xref:{url_guide_setup_dependencies}[Klayr SDK Dependencies] of the setup guide to set up all the required dependencies to follow this tutorial, if you have not done so already.
...
[source,bash]
----
klayr generate:asset lns reverse_lookup 2
----
...
[source,bash]
----
klayr generate:asset lns update_records 3
----
...
[source,bash]
----
./bin/run genesis-block:create --output  /home/USERNAME/.klayr/lns/config/default
----

Copy the following content to `~/.klayr/lns/config/default/config.json`:
...
In the LNS example application, the file is stored in the {url_sdkexamples_lns_secret}[klayr-name-service/lns/.secret/^] folder.
