...
:description: How to check, enable and disable forging on a Klayr node.
// Settings
:toc: preamble
:idprefix:
:idseparator: -
...
:url_backup_plugin: https://github.com/JesusTheHun/klayr-plugin-backup
:url_klayr_wallet: https://klayr.com/wallet
// Project URLs
:url_sdk_guides_forging: klayr-v3@ROOT::run-blockchain/forging.adoc
:url_sdk_protocol_puninshment: klayr-v3@ROOT::understand-blockchain/klayr-protocol/consensus-algorithm.adoc#punishment
:url_ref_cli: reference/cli.adoc
:url_reference_config: reference/config.adoc
:url_mgmt_accounts: management/account-management.adoc##how-to-voteunvote-delegates
...
To enable your node to forge for a particular delegate, firstly it is required to insert certain data into the config file under the `forging.delegates` array as described below:
...
Generate the necessary config object conveniently with the Klayr Core CLI.
...
[source,bash]
----
klayr-core forging:config --pretty
----
...
The command returns a list of delegates, based on the list under `forging.delegates` in the Klayr Core config, (based on the details added to the config in the previous step, <<adding-the-delegate-info-to-the-config>>).
...
Disable forging with the following command:
