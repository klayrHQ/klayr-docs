= Posting transactions with Klayr Service
:toc:

How to send transactions to a blockchain node via Klayr Service.

== Prepare the transaction object

To prepare the transaction, create the desired transaction object. This is made possible by using either the Klayr Core CLI, or alternatively using the API client:
...
const senderAddress = "klyt8ovj2shbxrtno8xqqt7cnmzzygdkbt6brnvmj";
...
An existing transaction object in its binary encoded format can be posted to Klayr Core via the Klayr Service either by using its HTTP or WebSocket APIs.
