...
:idseparator: -
// Projecgt URLs
:url_github_lip14: https://github.com/KlayrHQ/lips/blob/master/proposals/lip-0014.md#detecting-contradicting-block-headers
:url_wiki_pos: https://en.wikipedia.org/wiki/Proof_of_stake
:docs_general: klayr-v3@ROOT::

:url_protocol_blocks_rewards: {docs_general}understand-blockchain/klayr-protocol/blocks.adoc#rewards_and_fees
:url_protocol_dpos: {docs_general}understand-blockchain/klayr-protocol/consensus-algorithm.adoc#dpos
:url_protocol_bft: {docs_general}understand-blockchain/klayr-protocol/consensus-algorithm.adoc#klayr_bft
:url_protocol_delegate_selection: {docs_general}understand-blockchain/klayr-protocol/consensus-algorithm.adoc#delegate_selection
:url_protocol_fork_choice: {docs_general}understand-blockchain/klayr-protocol/consensus-algorithm.adoc#index-fork_choice_rule-1
:url_protocol_punishment: {docs_general}understand-blockchain/klayr-protocol/consensus-algorithm.adoc#punishment
:url_protocol_transactions_pom: {docs_general}understand-blockchain/klayr-protocol/transactions.adoc#pom
:url_protocol_transactions_delegate: {docs_general}understand-blockchain/klayr-protocol/transactions.adoc#delegate
:url_protocol_transactions_unlock: {docs_general}understand-blockchain/klayr-protocol/transactions.adoc#unlock
:url_protocol_transactions_vote: {docs_general}understand-blockchain/klayr-protocol/transactions.adoc#vote
...
:url_klayr_sdk: {docs_general}glossary.adoc#klayr-sdk
...
The xref:{url_klayr_sdk}[Klayr SDK] bootstraps a blockchain network that is based on the xref:{url_protocol_dpos}[Klayr DPoS] consensus algorithm.
...
[NOTE]
====
The DPoS used by Klayr is in fact more of a middle ground between {url_wiki_pos}[PoS] and DPoS.
...
More information about the delegate selection mechanism can be found in the xref:{url_protocol_delegate_selection}[Klayr Protocol].
...
This will start the unlocking procedure and the KLY will be ready for unlocking 2000 blocks later, (approximately 5 hours and 30 minutes).
...
Future improvements of the Klayr blockchain (particularly on the database level), could render this unlock transaction unnecessary.
+
[source,js]
----
const tx = await APIClient.transaction.create({
...
This means that for a given height, eventually all Klayr nodes agree on the same block.
...
Additionally, delegates can be punished by anyone in the network if they forge contradicting blocks, i.e., two blocks with consensus votes that violate the Klayr-BFT protocol (see the xref:{url_protocol_punishment}[Klayr protocol documentation] for details).
...
A xref:{url_protocol_transactions_pom}[delegate misbehavior report] transaction can be issued by anyone in the network who observes a violation of the Klayr-BFT consensus algorithm, as explained in <<discover>>.
