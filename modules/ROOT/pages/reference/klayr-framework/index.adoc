...
:description: The Klayr Framework overview provides a high-level synopsis of the Klayr Framework architecture, including its modules and components, how they communicate, and also how to change the default configuration.
:toc: preamble
:imagesdir: ../../assets/images
:page-no-next: true
:page-previous: /klayr-sdk/reference/klayr-elements/index.html
:page-previous-title: Klayr Elements

:url_github_lip05: https://github.com/KlayrHQ/lips/blob/master/proposals/lip-0005.md
:url_github_lip11: https://github.com/KlayrHQ/lips/blob/master/proposals/lip-0011.md
...
== What is the Klayr Framework?

The Klayr Framework is an application framework responsible for establishing and maintaining the interactions between the modules of a Klayr blockchain application.

The Klayr Framework aims to provide a consistent and intuitive interface between each module and component.
Currently the Klayr framework contains `chain`, `api` and `network` modules.

=== Architecture overview

The architecture of the Klayr Framework follows the research documented in {url_github_lip05}[LIP0005^].
...
The application logic can be configured as desired in the xref:{url_reference_config}[Klayr SDK configuration], see also <<configuration, Configuration of the Klayr Framework".

[[modules]]
=== Modules

Modules are located in the `modules` folder.
They are individual building blocks for the Klayr SDK.

==== Default modules

The default modules are shipped along with the Klayr SDK itself.
These modules constitute the minimum requirements to run a blockchain with the Klayr SDK.

===== List of default modules

* *API module:* This provides HTTP API endpoints, that enable users and other programs to interact with the Klayr blockchain through the API.
...
Custom modules can be plugged into the Klayr SDK and may offer new features/capabilities for the application, and replace the default modules functionalities where necessary.
They extend the existing instance with a specific, (and circumscribed) set of features.

[source,js]
----
const BaseModule = require('klayr-framework/src/modules/base_module')
...
The controller emits an event, `klayr:ready` which can be used to perform certain activities, which may be required to be performed when all the other modules are loaded.
...
To load a module in a child process, ensure the `ipc` is enabled in the xref:{url_reference_config}[config] and set the environment variable `KLAYR_CHILD_PROCESS_MODULES` with the module alias.
...
Multiple modules can be defined by using commas, as shown below: `KLAYR_CHILD_PROCESS_MODULES=httpApi,chain`.
...
The controller contains a complete list of events, which may occur in the modules of the Klayr SDK at any given time.
====
...
|klayr:ready |Triggered when the controller has finished initializing the modules, and each module has been successfully loaded.
|===
...
|klayr:getComponentConfig |A controller action to get the configuration of any component defined in the controller space.
|===
...
[[configuration]]
== Configuration of the Klayr Framework
