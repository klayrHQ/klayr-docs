...
:description: Technical references regarding the Delegate endpoints of the API client package of Klayr Elements. This consists of usage examples, available parameters and example responses.
:toc:
:v_core: 3.0.0
:url_klayr_core_api: {v_core}@klayr-core::reference/api.adoc

This is a resource for interacting with the `delegates` endpoint provided by the Klayr public API.
...
`options`: Please see the options in the following link: xref:{url_klayr_core_api}[Core API documentation^].
...
If no timestamps are provided, it will use the timestamps from the Klayr epoch to the current date.
