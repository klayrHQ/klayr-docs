...
:description: The Klayr Commander command reference lists all available commands of Klayr Commander as a technical reference. For each command this consists of all available options, a usage example and an example response.
:toc:
...
It displays the klayr address, publickey and privatekey that belong to the entered passphrase:
...
OPTIONS
  ...
  -p, --passphrase=passphrase  Specifies a source for your secret passphrase. Klayr Commander will
...
Commands relating to Klayr blocks.
...
Commands to get and manage configurations for Klayr Commander.
...
<2> These are the custom nodes which will connect to the Klayr Commander.
...
When `api.nodes` is empty, klayr-commander will connect to official Klayr seed nodes depending on the network specified in `api.network`.

If `api.nodes` is set to one or multiple nodes, Klayr Commander will ignore `api.network` and will make all requests to the specified Klayr node.
...
----
USAGE
  $ klayr config:set VARIABLE [VALUES]
...
Commands relating to Klayr delegates.
...
----
USAGE
  $ klayr delegate:voters USERNAMES
...
----
USAGE
  $ klayr delegate:votes ADDRESSES
...
VERSION
  klayr-commander/3.0.1 darwin-x64 node-v12.13.0
...
COMMANDS
  ...
  core                Install an instance of Klayr Core.
  ...
  signature           Commands relating to signatures for Klayr transactions from multisignature accounts.
...
OPTIONS
  ...
  -p, --passphrase=passphrase
      Specifies a source for your secret passphrase. Klayr Commander will prompt you for input if this
      option is not set.
      	Examples:
...
This command uses Klayr Elements passphrase module to encrypt a message you provide for a given public key using a randomly generated nonce.
...
OPTIONS
  ...
  -p, --passphrase=passphrase
      Specifies a source for your secret passphrase. Klayr Commander will prompt you for input if this option is not set.
      	Examples:
      	- --passphrase='my secret passphrase' (should only be used where security is not important)
...
----
USAGE
  $ klayr network-identifier GENESISPAYLOADHASH
...
Commands relating to Klayr nodes.

Uses official Klayr seed nodes, if no other nodes are provided in <<_config,config>>.

=== Forging

Update the forging status of a Klayr Core instance.

[source,bash]
----
USAGE
  $ klayr node:forging STATUS PUBLICKEY
...
OPTIONS
  ...
  -w, --password=password  Specifies a source for your secret password. Klayr Commander will prompt you
...
Get the network status from a Klayr Core instance.
...
OPTIONS
  ...
  -w, --password=password      Specifies a source for your secret password. Klayr Commander will prompt
...
Commands relating to Klayr transactions.
...
----
USAGE
  $ klayr transaction:broadcast [TRANSACTION]
...
EXAMPLES
  ...
  echo '{"type":0,"amount":"100",...}' | klayr transaction:broadcast
----
...
OPTIONS
  ...
  --networkIdentifier=networkIdentifier
      Network identifier defined for the network or main | test for the Klayr Network.
...
This command creates and signs a type 0 transaction, which will transfer a Klayr balance to a provided address if broadcast to the network.

[source,bash]
----
USAGE
  $ klayr transaction:create:transfer NONCE FEE AMOUNT ADDRESS

ARGUMENTS
  NONCE    Nonce of the transaction.
  FEE      Transaction fee in KLY.
  AMOUNT   Amount of KLY to send.
...
OPTIONS
  ...
  -p, --passphrase=passphrase            Specifies a source for your secret passphrase. Klayr Commander
...
----
USAGE
  $ klayr transaction:create:delegate NONCE FEE USERNAME
...
----
USAGE
  $ klayr transaction:create:vote NONCE FEE
...
----
USAGE
  $ klayr transaction:create:multisignature NONCE FEE
...
----
USAGE
  $ klayr transaction:create:pom NONCE FEE HEADER1 HEADER2
...
OPTIONS
  ...
  -p, --passphrase=passphrase            Specifies a source for your secret passphrase. Klayr Commander will prompt you
                                         for input if this option is not set.
                                         	Examples:
                                         	- --passphrase='my secret passphrase' (should only be used where security is
                                         not important)

  --networkIdentifier=networkIdentifier  Network identifier defined for the network or main | test for the Klayr
                                         Network.
...
----
USAGE
  $ klayr transaction:create:unlock NONCE FEE
...
OPTIONS
  ...
  --sender-id=sender-id
      Get transactions based by sender-id which is sender's klayr address'.
      	Examples:
...
----
USAGE
  $ klayr transaction:sign [TRANSACTION]
...
----
USAGE
  $ klayr transaction:verify [TRANSACTION]
