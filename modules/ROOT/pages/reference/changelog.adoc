...
:url_github_devnet_genesis_payload: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/sdk/src/samples/genesis_block_devnet.json#L11
:url_github_devnet_genesis_comid: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/sdk/src/samples/genesis_block_devnet.json#L9
...
The new parameter `networkIdentifier` is introduced with Klayr SDK version 3.x.
...
Due to the changes that have been introduced for the structure of transaction objects, the genesis block and it's transactions were created new for Klayr SDK version 3.0.

See the new genesis block on Github: https://github.com/KlayrHQ/klayr-sdk/blob/v3.0.2/sdk/src/samples/genesis_block_devnet.json

See the new devnet config: https://github.com/KlayrHQ/klayr-sdk/blob/v3.0.2/sdk/src/samples/config_devnet.json
...
To see what has changed in the database layer of Klayr SDK 3.x, compare the 2 figures `Database layer SDK 2.x`
...
[NOTE]
====
The list of API changes only comprises of the parts of the API that changed from Klayr SDK 2.x to 3.x.

To see the complete API specification for Klayr SDK 3.x, please refer to the xref:{url_config_reference}[Config reference].
====

Changes are grouped by the different API endpoints for the Klayr SDK.

=== Color legend
[cols=""]
|===
|{set:cellbgcolor:red}RED = Removed in Klayr SDK 3.x
|{set:cellbgcolor:green}GREEN = Added in Klayr SDK 3.x
|===
{set:cellbgcolor!}
...
|**totalForged** +
__required__|Total amount of KLY that have been forged in this Block.
...
Represents the total amount of Klayr (in Beddows) that the delegates' voters own.
...
__optional__|Offset value for results|integer (int32)|`0`
|**Query**|**recipientId** +
__optional__|Recipient's Klayr address|string (address)|
|{set:cellbgcolor:red}**Query**|**recipientPublicKey** +
__optional__|Recipient's public key|string (publicKey)|
|{set:cellbgcolor!}
**Query**|**senderId** +
__optional__|Sender's Klayr address|string (address)|
...
Broadhash consensus serves a vital function for the Klayr network in order to prevent forks.
...
__required__| Amount of Klayr to be transferred in this transaction.| string
|**fee** +
__required__|Transaction fee associated with this transaction.| string
|**recipientId** +
__required__|Klayr Address of the Recipients' account.| string (address)
|{set:cellbgcolor!}
**id** +
