...
:description: The Klayr Hello world tutorial teaches the user how to create a custom transaction type based on the BaseTransaction interface, and how to register it with the node application. It further details how to initially set up the Klayr SDK and how to send the newly created transaction type to the network.
:toc:
...
:url_explorer: https://explorer.klayr.io/
:url_github_hello: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/hello_world
:url_github_hello_client: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/hello_world/client/print_sendable_hello-world.js
:url_github_hello_index: https://github.com/KlayrHQ/klayr-sdk-examples/tree/development/hello_world/index.js
:url_github_hello_tx: https://github.com/KlayrHQ/klayr-sdk-examples/blob/development/hello_world/hello_transaction.js
:url_github_sdk_config: https://github.com/KlayrHQ/klayr-sdk/blob/{v_sdk}/sdk/src/samples/config_devnet.json
...
Welcome to the step-by-step guide of creating the Hello world application with the Klayr SDK.
...
The purpose of the Hello world application is to explain both how to implement and use custom transactions with the Klayr SDK.
...
The next step is to install the `klayr-sdk` package and add it to the project's dependencies.
...
[source,bash]
----
npm install klayr-sdk <1>
npm install @klayrhq/klayr-validator @klayrhq/klayr-cryptography @klayrhq/klayr-transactions @klayrhq/klayr-constants <2>
----

<1> Install the Klayr SDK as dependency for the node side.
<2> Install Klayr Elements dependencies for the client side scripts.

[IMPORTANT]
====
If a database has not previously been created then it is necessary to create one.
The default database name is `klayr_dev` The following command,  `createdb klayr_dev --owner klayr` should be used.
The default database user and password are `klayr` and `password`.
Both the user and password can be changed in the configuration of the Klayr SDK.
====
...
[source,bash]
----
psql -c "DROP DATABASE klayr_dev"
psql -c "CREATE DATABASE klayr_dev OWNER klayr"
----
--
...
[source,bash]
----
docker exec -ti klayr_sdk_db psql -h localhost -U klayr -d postgres -c "DROP DATABASE klayr_dev"
docker exec -ti klayr_sdk_db psql -h localhost -U klayr -d postgres -c "CREATE DATABASE klayr_dev OWNER klayr"
----
--
====
...
[source,js]
----
const { Application, genesisBlockDevnet, configDevnet } = require('klayr-sdk'); <1>
...
The necessary dependencies are required from the `klayr-sdk` package.
...
<3> In the case whereby a different user other than 'klayr' was given for access to the database klayr_dev, it will be necessary to update the username in the config.
...
TIP: In the case whereby the user wishes to change any of the values for `configDevnet`, please see the xref:{url_reference_config}[full list of configurations] for Klayr SDK and overwrite them as described in <<step7,paragraph 7>>.
...
....
$ node index.js | npx bunyan -o short
14:01:39.384Z  INFO klayr-framework: Booting the application with Klayr Framework(0.1.0)
14:01:39.391Z  INFO klayr-framework: Starting the app - helloWorld-blockchain-app
14:01:39.392Z  INFO klayr-framework: Initializing controller
14:01:39.392Z  INFO klayr-framework: Loading controller
14:01:39.451Z  INFO klayr-framework: Old PID: 7707
14:01:39.452Z  INFO klayr-framework: Current PID: 7732
14:01:39.467Z  INFO klayr-framework: Loading module klayr-framework-chain:0.1.0 with alias "chain"
14:01:39.613Z  INFO klayr-framework: Event network:bootstrap was subscribed but not registered to the bus yet.
14:01:39.617Z  INFO klayr-framework: Event network:bootstrap was subscribed but not registered to the bus yet.
14:01:39.682Z  INFO klayr-framework: Modules ready and launched
14:01:39.683Z  INFO klayr-framework: Event network:event was subscribed but not registered to the bus yet.
14:01:39.684Z  INFO klayr-framework: Module ready with alias: chain(klayr-framework-chain:0.1.0)
14:01:39.684Z  INFO klayr-framework: Loading module klayr-framework-network:0.1.0 with alias "network"
14:01:39.726Z  INFO klayr-framework: Blocks 1886
14:01:39.727Z  INFO klayr-framework: Genesis block matched with database
14:01:39.791Z ERROR klayr-framework: Error occurred while fetching information from 127.0.0.1:5000
14:01:39.794Z  INFO klayr-framework: Module ready with alias: network(klayr-framework-network:0.1.0)
14:01:39.795Z  INFO klayr-framework: Loading module klayr-framework-http-api:0.1.0 with alias "http_api"
14:01:39.796Z  INFO klayr-framework: Module ready with alias: http_api(klayr-framework-http-api:0.1.0)
14:01:39.797Z  INFO klayr-framework:
...
14:01:39.799Z  INFO klayr-framework:
...
14:01:39.800Z  INFO klayr-framework: App started...
14:01:39.818Z  INFO klayr-framework: Validating current block with height 1886
14:01:39.819Z  INFO klayr-framework: Loader->validateBlock Validating block 10258884836986606075 at height 1886
14:01:40.594Z  INFO klayr-framework: Klayr started: 0.0.0.0:4000
14:01:40.600Z  INFO klayr-framework: Verify->verifyBlock succeeded for block 10258884836986606075 at height 1886.
14:01:40.600Z  INFO klayr-framework: Loader->validateBlock Validating block succeed for 10258884836986606075 at height 1886.
14:01:40.600Z  INFO klayr-framework: Finished validating the chain. You are at height 1886.
14:01:40.601Z  INFO klayr-framework: Blockchain ready
14:01:40.602Z  INFO klayr-framework: Loading 101 delegates using encrypted passphrases from config
14:01:40.618Z  INFO klayr-framework: Forging enabled on account: 8273455169423958419L
14:01:40.621Z  INFO klayr-framework: Forging enabled on account: 12254605294831056546L
14:01:40.624Z  INFO klayr-framework: Forging enabled on account: 14018336151296112016L
14:01:40.627Z  INFO klayr-framework: Forging enabled on account: 2003981962043442425L
[...]
....
...
If the account contains an adequate enough balance to process the `HelloTransaction` transaction, (the fee is set to 1 KLY by default), the new "hello" property will appear into the account’s asset field.
...
class HelloTransaction extends BaseTransaction {
    ...
    * The first 10 types, from 0-9 is reserved for the default Klayr network functions.
    */
    static get TYPE () {
        return 20;
    }
    ...
    /**
    * Set the `HelloTransaction` transaction FEE to 1 KLY.
...
<4> If a different user other than 'klayr' was given for access to the database klayr_dev, then it is necessary to update the username in the config.
...
const { EPOCH_TIME } = require('@klayrhq/klayr-constants');
...
TIP: For more configuration options, please see the xref:{url_reference_config}[full list of configurations] for Klayr SDK.
...
<4> In the case whereby a different user than 'klayr' was given, to access to the database klayr_dev, it is necessary to update the username in the config.
...
If so required, a frontend application can be designed such as the {url_explorer}[Klayr Explorer^], which displays the user's assets inside of their account page.
