...
:description: The Klayr Supply Chain tutorial teaches the user how to build a blockchain application with IoT devices which can communicate with the network. It uses a raspberry pi with a light sensor to track the transport of a packet on the blockchain as part of a supply chain tracking system. It further details how to implement a simple trust system, and how to finally publish the application.
:toc:
...
For convenient usage of the system, a client application called Klayr Transport will be provided that visualizes all relevant information regarding the supply chain tracking system. In turn the Klayr Transport application enables the users to interact with the system by posting custom transactions.
...
image::transport-uml.svg[Flow chart of Klayr Transport]
